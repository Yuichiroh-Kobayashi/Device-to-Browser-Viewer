...
/**
 * Thin, audio-specific application-owned d2b-stream/0.1 control state machine
 * for the pcm-audio profile: CONNECTED -> READY -> STREAMING -> READY/CLOSED.
 * This is new code modeled closely on
 * ../../protocol/session-adapter.js's reserve/commit/rollback pattern for
 * outbound controls (that pattern is worth replicating), restricted to
 * pcm-audio instead of vi-measurement, and paired with a pcm-audio model
 * instead of the V/I StreamModel. src/protocol/session-adapter.js itself is
 * not imported or edited.
 *
 * The copied reference parser (control-parser.js, decoder-state.js,
 * decoder.js) validates wire syntax and frame bytes; this adapter only owns
 * transition timing and uses the reference decoder after negotiation.
 */
export class PcmSessionAdapter {
  constructor(model, { onChange = null } = {}) {
    if (!model || typeof model.prepareDecodedFrame !== "function") throw new TypeError("a PcmAudioModel is required");
    this.model = model;
  ...
  _applyControl(message, direction) {
    ...
    if (message.type === "stream_started") {
      ...
      this.model.beginStream({
        streamId: message.stream_id,
        profile: request.profile,
        sampleRate: request.parameters.sample_rate,
        samplesPerFrame: request.parameters.samples_per_frame,
      });
  ...
  handleBinary(buffer) {
    ...
    try {
      // Both methods return candidates. Neither reference state nor model
      // state is changed until model validation has completed successfully,
      // so a rejected/malformed frame leaves both untouched (transactional).
  ...
  _assertStartAllowed(message) {
    ...
    if (message.profile !== "pcm-audio") throw new TypeError("this spike adapter supports only pcm-audio");
  }
